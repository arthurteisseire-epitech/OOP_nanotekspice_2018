...
TEST(ComponentNor, Init)
{
	nts::ComponentNor orComp("or");
...
{
	nts::ComponentNor componentOr("or");
...
TEST(ComponentNor, LocalCompute)
{
	...
	testLocalCompute(nts::TRUE, nts::UNDEFINED, nts::FALSE);
	testLocalCompute(nts::UNDEFINED, nts::TRUE, nts::FALSE);
...
{
	nts::ComponentNor norComp("nor");

	norComp.getPin(0)->setState(in1State);
	norComp.getPin(1)->setState(in2State);
	norComp.localCompute();
	EXPECT_EQ(norComp.getPin(0)->getState(), in1State);
	EXPECT_EQ(norComp.getPin(1)->getState(), in2State);
	EXPECT_EQ(norComp.getPin(2)->getState(), expected);
}

TEST(ComponentNor, Compute)
{
	...
	testCompute(nts::TRUE, nts::UNDEFINED, nts::FALSE);
	testCompute(nts::UNDEFINED, nts::TRUE, nts::FALSE);
