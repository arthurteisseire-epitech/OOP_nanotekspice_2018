...
void nts::AComponent::setLink(size_t pin, nts::IComponent &other, size_t otherPinIdx)
{
	...
	std::shared_ptr<PinInput> inputPin = findInputPin(localPin, otherPin);
	std::shared_ptr<PinOutput> outputPin = findOutputPin(localPin, otherPin);

	_pins[pin] = otherPin;
	inputPin->link(outputPin);
}

std::shared_ptr<nts::PinInput> nts::AComponent::findInputPin(std::shared_ptr<nts::IPin> &pin1,
...
std::shared_ptr<nts::PinOutput> nts::AComponent::findOutputPin(std::shared_ptr<nts::IPin> &pin1,
                                                             const std::shared_ptr<nts::IPin> &pin2)
{
	if (pin1->getType() == pin2->getType())
		throw std::exception();
